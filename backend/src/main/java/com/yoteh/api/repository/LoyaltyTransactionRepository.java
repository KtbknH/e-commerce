...
public interface LoyaltyTransactionRepository extends JpaRepository<LoyaltyTransaction, UUID> {
    ...
    @Query("SELECT COALESCE(SUM(lt.points), 0) FROM LoyaltyTransaction lt WHERE lt.user.id = :userId AND lt.type = 'EARN'")
    int sumEarnedPointsByUserId(@Param("userId") UUID userId);

    @Query("SELECT COALESCE(SUM(lt.points), 0) FROM LoyaltyTransaction lt WHERE lt.user.id = :userId AND lt.type = 'REDEEM'")
