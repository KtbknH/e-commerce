...
public interface OrderRepository extends JpaRepository<Order, UUID> {
    ...
    @Query("SELECT COALESCE(SUM(o.total), 0) FROM Order o WHERE o.deletedAt IS NULL AND o.status = :status")
    BigDecimal sumTotalByStatus(@Param("status") OrderStatus status);

    @Query("SELECT COALESCE(SUM(o.total), 0) FROM Order o WHERE o.deletedAt IS NULL " +
            "AND o.status IN ('PAID', 'PREPARING', 'SHIPPED', 'DELIVERED') " +
            "AND o.createdAt >= :from AND o.createdAt <= :to")
    BigDecimal sumRevenueForPeriod(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
