...
public interface ReviewRepository extends JpaRepository<Review, UUID> {
    ...
    Page<Review> findByProductIdAndIsApprovedTrueAndDeletedAtIsNullOrderByCreatedAtDesc(UUID productId, Pageable pageable);
    ...
    @Query("SELECT AVG(r.rating) FROM Review r WHERE r.product.id = :productId AND r.isApproved = true AND r.deletedAt IS NULL")
    Double getAverageRatingByProductId(@Param("productId") UUID productId);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.product.id = :productId AND r.isApproved = true AND r.deletedAt IS NULL")
